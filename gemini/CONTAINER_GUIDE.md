# AI Agent Tutorial - Container Guide (Gemini)

This guide shows you how to run the Gemini AI agent tutorial in a container using Podman or Docker.
...
# Or using Docker
docker build -t ai-agent-gemini -f Containerfile .
```
...
podman run -it --rm -e GOOGLE_API_KEY="your-api-key-here" ai-agent-gemini

# Or with Docker
docker run -it --rm -e GOOGLE_API_KEY="your-api-key-here" ai-agent-gemini
```
...
Start a Python session with the Gemini SDK pre-loaded:
...
```bash
podman run -it --rm --env-file .env ai-agent-gemini
```
...
```bash
podman run -it --rm ai-agent-gemini
```
...
```bash
podman run -it --rm -e GOOGLE_API_KEY="AI..." ai-agent-gemini
```
...
If you get "No module named 'google.generativeai'", rebuild the container:

```bash
podman build --no-cache -t ai-agent-gemini -f Containerfile .
```
...
Your containerized Gemini agent tutorial environment is ready to use.
